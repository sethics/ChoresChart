...
const ChoresChart = () => {
  ...
  const participantsRef = db ? db.collection('choreChart').doc('participants') : null;
  const choresRef = db ? db.collection('choreChart').doc('chores') : null;
  ...
  useEffect(() => {
    ...
    if (db) {
      const unsubParticipants = participantsRef.onSnapshot(doc => {
      ...
      const unsubChores = choresRef.onSnapshot(doc => {
        if (doc.exists) {
          setChores(doc.data());
          setLastSynced(new Date());
        }
  ...
  const saveParticipants = async (newParticipants) => {
    try {
      ...
      if (db) {
        await participantsRef.set({ list: newParticipants });
        setLastSynced(new Date());
      }
    } catch (error) {
  ...
  const refreshData = async () => {
    if (!db) {
      alert("Cannot refresh data without Firebase connection");
      return;
    }
